...
inline void reduce_achstemp(int n1, int number_bands, int* inv_igp_index, int ncouls, GPPComplex  *aqsmtemp, GPPComplex *aqsntemp, GPPComplex *I_eps_array, GPPComplex achstemp,  int* indinv, int ngpown, double* vcoul)
{
    double to1 = 1e-6;
...
int main(int argc, char** argv)
{
   ...
   for(int i=0; i<ncouls; i++)
    ...
    d_till_nvbandKernel(d_asxtemp, d_inv_igp_index, d_vcoul, d_wtilde_array, d_aqsmtemp, d_aqsntemp, d_I_eps_array, d_wx_array, nvband, ncouls, ngpown);
...
//       noFlagOCCSolver(n1, nvband, ngpown, ncouls, inv_igp_index, wx_array, wtilde_array, aqsmtemp, aqsntemp, I_eps_array, vcoul, achtemp_re, achtemp_im); 
 ...
 //   d_achtemp_kernel(number_bands, ngpown, ncouls, wtilde_array, inv_igp_index, aqsmtemp, aqsntemp, I_eps_array, acht_n1_loc, wx_array, vcoul, achtemp_re, achtemp_im);
